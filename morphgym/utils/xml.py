...
def reformat_and_save_xml(tree,folder):
    """Reformat tree."""
    #TODO Need attributes checking!
    tree_str = etree.tostring(tree,encoding='utf-8')
    md5_name = hashlib.md5(tree_str).hexdigest()

    path = os.path.join(folder,f"{md5_name}.xml")
    tree.write(
...
def site_elem(name, pos, s_class, size=None, fromto=None, type_=None):
    ...
    attr_dict = {"element": name, "code": s_class}
    if pos:
...
def actuator_elem(name, gear):
    ...
    return etree.Element(
        "motor", {"joint": name, "gear": "{}".format(gear), "element": name},
    )
...
def body_elem(name, pos):
    ...
    return etree.Element("body", {"element": name, "pos": arr2str(pos)})


def sensor_elem(type_, name, site):
    """Helper function to create sensor element."""

    return etree.Element(type_, {"element": name, "site": site})
...
def exclude_elem(name1, name2):
    """Exclude contact between geom of element 1 and 2."""
    attrs = {
        "element": "{}:{}".format(name1, name2),
        "body1": name1,
        "body2": name2
    }
