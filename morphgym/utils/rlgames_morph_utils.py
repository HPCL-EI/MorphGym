...
import torch
from core.tasks import task_map
from rl_games.algos_torch import torch_ext
from rl_games.common import env_configurations, vecenv
from rl_games.common.algo_observer import AlgoObserver


def get_rlgames_morph_env_creator(
        # used to create the vec base
        task_config: dict,
        task_name: str,
        sim_device: str,
        rl_device: str,
    ...
    """Parses the configuration parameters for the environment base and creates a VecTask

    Args:
        task_config: environment configuration.
        task_name: Name of the base, used to evaluate based on the imported element (eg 'Trifinger')
        sim_device: The type of base device, eg 'cuda:0'
    ...
    def create_rlgpu_env(_sim_device=sim_device, _rl_device=rl_device, **kwargs):
        """
        Creates the base from configurations and wraps it using RL-games wrappers if required.
        """
        ...
        # create native base and pass custom config
        env = task_map[task_name](
            cfg=task_config,
            sim_device=_sim_device,
            graphics_device_id=graphics_device_id,
            headless=headless,
            morph_cfg=kwargs['morph_cfg']
        )
...
class RLGPUAlgoObserver(AlgoObserver):
    """Allows us to log stats from the base along with the algorithm running stats. """
    ...
    def process_infos(self, infos, done_indices):
        ...
        if isinstance(infos, dict):
            ...
            if len(infos) > 0 and isinstance(infos, dict):  # allow direct logging from base
                self.direct_info = {}
