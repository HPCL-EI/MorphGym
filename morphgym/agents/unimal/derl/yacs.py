...
class CfgNode(dict):
    ...
    def register_renamed_key(self, old_name, new_name, message=None):
        ...
        _assert_with_logging(
            ...
            "key {} is already registered as a renamed config key".format(old_name),
        )
        value = new_name
        if message:
        ...
        Args:
            cfg_file_obj_or_str (str or file):
                ...
                - A file object backed by a Python source file that exports an attribute
                  "config" that is either a dict or a CfgNode
    ...
    def _load_cfg_py_source(cls, filename):
        ...
        _assert_with_logging(
            ...
            "Python module from file {} must have 'config' attr".format(filename),
        )
        VALID_ATTR_TYPES = {dict, CfgNode}
        _assert_with_logging(
            type(module.cfg) in VALID_ATTR_TYPES,
            "Imported module 'config' attr must be in {} but is {} instead".format(
