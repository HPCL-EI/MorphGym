...
import morphgym
from morphgym.utils.xml import reformat_and_save_xml
from ..config import cfg
from ..utils import file as fu
from ..utils import geom as gu
from ..utils import mjpy as mu
from ..utils import sample as su
...
class SymmetricUnimal:
    ...
    def __init__(self, id_, init_path=None, seed=0):
        self.id = id_
        random.seed(seed)
    ...
    def _construct_head(self):
        ...
        if cfg.HFIELD.DIM == 1:
            head.append(xu.joint_elem("rootx", "slide", axis=[1, 0, 0]))
            head.append(xu.joint_elem("rootz", "slide", axis=[0, 0, 1]))
            head.append(xu.joint_elem("rooty", "hinge", axis=[0, 1, 0]))
        else:
            head.append(xu.joint_elem("root"))
    ...
    def save(self):
        self._before_save()
        os.makedirs(os.path.join(cfg.OUT_DIR, "xml"),exist_ok=True)
        xml_folder = os.path.join(cfg.OUT_DIR, "xml")
        xml_path = os.path.join(cfg.OUT_DIR, "xml", "{}.xml".format(self.id))

        path = reformat_and_save_xml(self.tree, xml_folder)
        if self.parent_id:
        ...
        os.makedirs(os.path.join(cfg.OUT_DIR, "unimal_init"),exist_ok=True)
