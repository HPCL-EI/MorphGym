...
class EnvViewer(MjViewer):
    ...
    def add_goal_marker(self):
        for marker in self.env.metadata["markers"]:
    ...
    def get_camera_name(self):
        camera_id = self.cam.fixedcamid
        if camera_id == -1:
            return "Free Camera"
        else:
            return self.env.sim.model.camera_id2name(self.cam.fixedcamid)
    ...
    def run(self, once=False):
        while True:
            ...
            self.add_overlay(
                const.GRID_TOPLEFT,
                "Camera Name",
                "{}".format(self.get_camera_name()),
            )
            self.add_overlay(
                const.GRID_TOPRIGHT, "Apply action", "A (-0.05) / Z (+0.05)"
            )
            self.add_overlay(
                const.GRID_TOPRIGHT,
