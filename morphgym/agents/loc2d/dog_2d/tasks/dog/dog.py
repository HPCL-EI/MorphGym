...
from core.tasks.dog.dog_fsm import DogFSM
from isaacgymenvs.utils.torch_jit_utils import *


class Dog(Locomotion):
    def __init__(self, cfg, sim_device, graphics_device_id, headless, morph_cfg):
        super().__init__(cfg=cfg, sim_device=sim_device, graphics_device_id=graphics_device_id, headless=headless,
                         morph_cfg=morph_cfg, fsm_class=DogFSM)
...
def compute_reward_and_reset(
    ...
    # type: (Tensor, Tensor, Tensor,  float, Tensor, Tensor,  Tensor, float, float, float,Tensor,Tensor,float,float,Tensor,float) -> Tuple[Tensor, Tensor,Tensor]
    ...
    heading_reward = torch.where(obs_buf[:, 12] > 0.8, heading_weight_tensor, heading_weight * obs_buf[:, 12] / 0.8) / \
                     torch.cos(torch.tensor(plane_slope, device=heading_weight_tensor.device))

    # punish for undesired posture
    posture_angles = obs_buf[:, 7:10]
    if plane_slope != 0:
        posture_angles[:, 1] -= plane_slope
