...
class Locomotion(VecTask):
    ...
    def __init__(self, cfg, sim_device, graphics_device_id, headless, morph_cfg, fsm_class):
        ...
        # Creating some basic tensors, including sim tensors from Issac Gym and locomotion base related tensors.
    ...
    def _parse_cfg(self, cfg, morph_cfg):
        ...
        # base
        self.randomization_params = self.cfg["base"]["randomization_params"]
        self.randomize = self.cfg["base"]["randomize"]
        ...
        self.start_pos = self.cfg["base"]["start_pos"]

        # elif self.plane_slope == 0:
        #     self.init_height_offset = self.config["base"]["init_height_offset"]
        # else:
        #     # self.init_height_offset = self.config["base"]["slope_height_offset"] * self.plane_slope / np.abs(self.plane_slope)
        #     self.init_height_offset = self.config["base"]["init_height_offset"]

        self.max_episode_length = self.cfg["base"]["episodeLength"]
        self.env_spacing = self.cfg["base"]["envSpacing"]
        self.power_scale = self.cfg["base"]["powerScale"]

        self.termination_height = self.cfg["base"]["terminationHeight"]

        self.debug_viz = self.cfg["base"]["enableDebugVis"]
        self.plane_static_friction = self.cfg["base"]["plane"]["staticFriction"]
        self.plane_dynamic_friction = self.cfg["base"]["plane"]["dynamicFriction"]
        self.plane_restitution = self.cfg["base"]["plane"]["restitution"]
    ...
    def _create_envs(self):
        print(f'num envs {self.num_envs} base spacing {self.env_spacing}')
        ...
        spacing = self.cfg["base"]['envSpacing']
        ...
        for i in range(len(self.envs)):
            ...
            if i == 1:
                rigid_x += self.cfg["base"]['envSpacing'][0] * 2
    ...
    def _create_task_tensors(self):
        """Creating locomotion base related tensors."""
